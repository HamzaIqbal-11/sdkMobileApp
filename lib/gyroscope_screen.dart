  ...
  Future<void> _sendToBackend() async {
    ...
    try {
      ...
      if (response.statusCode >= 200 && response.statusCode < 300) {
        setState(() => _sent = true);
        _showSnack('Gyroscope data sent!', Colors.green.shade700);
      } else {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        elevation: 0,
        title: const Text('Gyroscope', style: TextStyle(color: Colors.cyanAccent, fontWeight: FontWeight.bold)),
      ...
      body: Padding(
        ...
        child: !widget.hasGyro
            ? Center(
                child: Column(
                  mainAxisSize: MainAxisSize.min,
                  children: [
                    Icon(Icons.sensors_off, color: Colors.white24, size: 64),
                    const SizedBox(height: 16),
                    const Text('Gyroscope not available\non this device', style: TextStyle(color: Colors.white24, fontSize: 16), textAlign: TextAlign.center),
                  ],
                ),
              )
            : Column(
                children: [
                  ...
                  Icon(Icons.screen_rotation, color: Colors.cyanAccent.withOpacity(0.3), size: 80),
                  ...
                  _SensorCard('X-Axis', _gx, Colors.redAccent),
                  const SizedBox(height: 14),
                  _SensorCard('Y-Axis', _gy, Colors.greenAccent),
                  const SizedBox(height: 14),
                  _SensorCard('Z-Axis', _gz, Colors.blueAccent),
                  const SizedBox(height: 30),
                  SizedBox(
    ...
    return Container(
      ...
      child: Row(
        ...
        children: [
          ...
          Text(value.toStringAsFixed(6), style: TextStyle(color: color, fontSize: 18, fontWeight: FontWeight.bold, fontFamily: 'monospace')),
        ],
      ),
    );
  }
